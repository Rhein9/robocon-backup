...
private:
    ...
    rclcpp::TimerBase::SharedPtr timer_pub_map_;
                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    // Keep the 3D point-cloud map on its own topic so it does not conflict
    // with Nav2's 2D occupancy-grid /map topic.
    pub_map_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_map", map_qos);
    ...
    // Republish the static map periodically so RViz and late subscribers always receive it.
    timer_pub_map_ = this->create_wall_timer(std::chrono::seconds(1), [this]()
                                             {
        if (!pcd_map_coarse_ || pcd_map_coarse_->IsEmpty())
        {
            return;
        }

        sensor_msgs::msg::PointCloud2 map_msg;
        open3d_conversions::open3dToRos(*pcd_map_coarse_, map_msg);
    ...
    if (!GetTfTransformToMatrix("base_link", "imu_link", mat_imulink2baselink_))
    {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Failed to get TF base_link <- imu_link at startup, fallback to identity.");
    }
    ...
    if (!GetTfTransformToMatrix("motion_link", "base_link", mat_baselink2motionlink_))
    {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Failed to get TF motion_link <- base_link at startup, fallback to identity.");
    }
...
void GloabalLocalization::CallbackBaselink2Odom(const nav_msgs::msg::Odometry::SharedPtr baselink2odom)
{
    ...
    if (loc_initialized_)
    {
        ...
        // Do not broadcast map -> motion_link here.
        // motion_link is already a fixed child of base_link in the static TF tree.
        // Broadcasting it again would give motion_link two parents and break the TF logic.
